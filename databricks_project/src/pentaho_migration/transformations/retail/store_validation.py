"""Transformation module for missing Pentaho file: Store_Validation.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.store_validation")

from pyspark.sql import types as T


SOURCE_KTR = 'Store_Validation.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Store_Validation.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
