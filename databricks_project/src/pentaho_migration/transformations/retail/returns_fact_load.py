"""Transformation module for missing Pentaho file: Returns_Fact_Load.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.returns_fact_load")

from pyspark.sql import types as T


SOURCE_KTR = 'Returns_Fact_Load.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Returns_Fact_Load.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
