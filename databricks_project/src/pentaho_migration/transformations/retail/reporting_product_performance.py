"""Transformation module for missing Pentaho file: Reporting_Product_Performance.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.reporting_product_performance")

from pyspark.sql import types as T


SOURCE_KTR = 'Reporting_Product_Performance.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Reporting_Product_Performance.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
