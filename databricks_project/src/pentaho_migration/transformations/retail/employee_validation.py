"""Transformation module for missing Pentaho file: Employee_Validation.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.employee_validation")

from pyspark.sql import types as T


SOURCE_KTR = 'Employee_Validation.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Employee_Validation.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
