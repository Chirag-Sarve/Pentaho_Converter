"""Transformation module for missing Pentaho file: Employee_Dimension_Load.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.employee_dimension_load")

from pyspark.sql import types as T


SOURCE_KTR = 'Employee_Dimension_Load.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Employee_Dimension_Load.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
