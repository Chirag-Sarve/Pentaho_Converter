"""Transformation module for missing Pentaho file: FX_Rate_Lookup_Prep.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.fx_rate_lookup_prep")

from pyspark.sql import types as T


SOURCE_KTR = 'FX_Rate_Lookup_Prep.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('FX_Rate_Lookup_Prep.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
