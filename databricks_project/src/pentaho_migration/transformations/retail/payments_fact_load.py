"""Transformation module for missing Pentaho file: Payments_Fact_Load.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.payments_fact_load")

from pyspark.sql import types as T


SOURCE_KTR = 'Payments_Fact_Load.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Payments_Fact_Load.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
