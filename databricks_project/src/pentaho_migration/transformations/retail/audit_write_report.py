"""Transformation module for missing Pentaho file: Audit_Write_Report.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.audit_write_report")

from pyspark.sql import types as T


SOURCE_KTR = 'Audit_Write_Report.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Audit_Write_Report.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
