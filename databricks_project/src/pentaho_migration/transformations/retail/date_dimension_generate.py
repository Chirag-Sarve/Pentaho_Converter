"""Transformation module for missing Pentaho file: Date_Dimension_Generate.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.date_dimension_generate")

from pyspark.sql import types as T


SOURCE_KTR = 'Date_Dimension_Generate.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Date_Dimension_Generate.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
