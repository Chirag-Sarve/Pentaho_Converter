"""Transformation module for missing Pentaho file: Region_Dimension_Load.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.region_dimension_load")

from pyspark.sql import types as T


SOURCE_KTR = 'Region_Dimension_Load.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Region_Dimension_Load.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
