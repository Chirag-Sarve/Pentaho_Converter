"""Transformation module for missing Pentaho file: Category_Cleansing.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.category_cleansing")

from pyspark.sql import types as T


SOURCE_KTR = 'Category_Cleansing.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Category_Cleansing.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
