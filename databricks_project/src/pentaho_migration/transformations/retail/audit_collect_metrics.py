"""Transformation module for missing Pentaho file: Audit_Collect_Metrics.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.audit_collect_metrics")

from pyspark.sql import types as T


SOURCE_KTR = 'Audit_Collect_Metrics.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Audit_Collect_Metrics.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
