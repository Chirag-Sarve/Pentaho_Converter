"""Transformation module for missing Pentaho file: Payments_Cleansing.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.payments_cleansing")

from pyspark.sql import types as T


SOURCE_KTR = 'Payments_Cleansing.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Payments_Cleansing.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
