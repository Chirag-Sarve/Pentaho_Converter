"""PySpark module migrated from Pentaho transformation: TR_Sales_Audit.

Source: C:/Users/Prateek.Kotian/Desktop/Pentaho/Retail & E-commerce/Retail_ETL_Project/transformations/utilities/TR_Sales_Audit.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.tr_sales_audit")



def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    """Execute Pentaho transformation ``TR_Sales_Audit`` step-for-step."""
    ...
    data = [('fact_sales', 'SALES_ETL', 'SUCCESS')]
    ...
    df_Read_Extract_Metrics = (
        ...
        .load(f'{data_dir}/TR_Sales_Extract_.log')
    )
    ...
    df_Read_Fact_Metrics = (
        ...
        .load(f'{data_dir}/TR_FactSales_Load_.log')
    )
    ...
    df_Read_Reject_File = (
        ...
        .option('quote', '"')
        .option('inferSchema', False)
        .schema('order_item_id STRING, reject_reason STRING')
        .load(f'{data_dir}/sales_rejects_.csv')
    )
    ...
    # field 'module_name' from variable string 'Load_Sales_Data'
    ...
    if _dbu__module_name_resolved is not None and hasattr(_dbu__module_name_resolved, 'widgets'):
        try:
            _module_name_resolved = _dbu__module_name_resolved.widgets.get('Load_Sales_Data')
        except Exception:
            _module_name_resolved = None
    if _module_name_resolved in (None, ''):
        import os as _os__module_name_resolved
        _module_name_resolved = _os__module_name_resolved.environ.get('Load_Sales_Data')
    if _module_name_resolved in (None, ''):
        try:
            _module_name_resolved = spark.conf.get('pentaho.var.Load_Sales_Data')
        except Exception:
            _module_name_resolved = None
    if _module_name_resolved in (None, ''):
    ...
    df_Sum_All_Fact_Actions = df_Read_Fact_Metrics.groupBy().agg(_sum(col("action_count")).alias('rows_written_all'), _sum(col("action_count")).alias('rows_inserted_all'), _sum(col("action_count")).alias('rows_updated_all'))
    ...
    # Step: Valid Metrics Rows? (FilterRows) [failed]
    # Filter Rows: Valid Metrics Rows?
    df_Sum_Valid_Rows = df_Read_Validation_Metrics.filter((col("validation_status") == lit('VALID')))
    df_Non_Valid_Metrics = df_Read_Validation_Metrics.filter(~((col("validation_status") == lit('VALID'))))
    df_Valid_Metrics_Rows? = df_Sum_Valid_Rows
    ...
    # Step: Non Valid Metrics (Dummy) [converted]
    # Dummy: Non Valid Metrics
    # Pass-through step - DataFrame unchanged
    df_Dummy_Non_Valid_Metrics = df_Non_Valid_Metrics

    # Step: Sum Valid Rows (MemoryGroupBy) [failed]
    # Memory Group By: Sum Valid Rows
    # NOTE: Pentaho Memory Group By aggregates entirely in JVM heap; Spark uses distributed groupBy().agg() — memory pressure shifts to executors, and result ordering / early-partial-agg timing may differ.
    df_Sum_Valid_Rows = df_Valid_Metrics_Rows?.groupBy().agg(_sum(col("row_count")).alias('rows_valid'))

    # Step: Seed Audit Record (Constant) [converted]
    # Add Constants: Seed Audit Record
    df_Seed_Audit_Record = df_Capture_Audit_Timestamps
    df_Seed_Audit_Record = df_Seed_Audit_Record.withColumn("object_name", lit('fact_sales'))
    # preserved.object_name: length='-1', precision='-1'
    df_Seed_Audit_Record = df_Seed_Audit_Record.withColumn("layer", lit('SALES_ETL'))
    ...
    df_Seed_Audit_Record = df_Seed_Audit_Record.withColumn("rows_written", lit('0'))
    ...
    _mapped_df_Write_Audit_Table = df_Audit_Checksum_Fingerprint.select(col('object_name'), col('layer'), col('rows_read'), col('rows_written'), col('rows_rejected'), col('rows_inserted'), col('rows_updated'), col('execution_time'), col('error_count'), col('status'), col('batch_id'), col('run_id'), col('audit_start_ts'), col('audit_end_ts'), col('audit_checksum'))
    df_Write_Audit_Table = _mapped_df_Write_Audit_Table
    write_delta(
        df_Write_Audit_Table,
        f"{catalog}.{schema}.etl_audit_sales",
        mode='append',
        partition_by=config.get('partition_by') or [],
        target_files=config.get('target_files'),
        spark=spark,
    )
    log_event(_LOG, "delta_write", table=f"{catalog}.{schema}.etl_audit_sales", mode='append')

    # Step: Write Audit CSV (TextFileOutput) [converted]
    # Pentaho step: Write Audit CSV (type: TextFileOutput)
    # Pentaho filename: /audit/load_audit/sales_audit_
    ...
    _out_df_Write_Audit_CSV = df_Write_Audit_CSV.select('object_name', 'layer', 'rows_read', 'rows_written', 'rows_rejected', 'rows_inserted', 'rows_updated', 'execution_time', 'error_count', 'status', 'batch_id', 'run_id', 'audit_start_ts', 'audit_end_ts', 'audit_checksum')
    ...
    writer.mode('overwrite').save(f'{data_dir}/sales_audit_.csv')

    # Step: Write Execution Log (TextFileOutput) [converted]
    # Pentaho step: Write Execution Log (type: TextFileOutput)
    # Pentaho filename: /logs/execution/sales/TR_Sales_Audit_
    # NOTE: Spark CSV/text writers create a directory at this path (not a single flat file); subsequent Text File Input must load the same path
    # NOTE: empty input DataFrames write zero data files; missing parent paths are usually created by the filesystem
    # INFO: preserved split_every='0' — use repartition/coalesce to control output file count
    # INFO: create_parent_folder=Y — Spark/DBFS typically creates parent folders
    ...
    _out_df_Write_Execution_Log = df_Write_Execution_Log.select('object_name', 'layer', 'rows_read', 'rows_written', 'rows_rejected', 'rows_inserted', 'rows_updated', 'execution_time', 'error_count', 'status', 'batch_id', 'run_id', 'audit_start_ts', 'audit_end_ts', 'audit_checksum')
    writer = _out_df_Write_Execution_Log.write.format("csv")
    writer = writer.option("header", True)
    writer = writer.option("sep", ',')
    writer = writer.option("quote", '"')
    writer = writer.option("encoding", 'UTF-8')
    writer.mode('overwrite').save(f'{data_dir}/TR_Sales_Audit_.log')
    ...
    .mode('overwrite') \
    .json(
        f'{data_dir}/sales_audit__exec.json'
    )
    ...
    # preserved.log_message='AUDIT | EVENT=AUDIT_COMPLETE | TRANS=TR_Sales_Audit | ROWS_READ=${rows_read} | ROWS_WRITTEN=${rows_written} | ROWS_REJECTED=${rows_rejected} | ROWS_INSERTED=${rows_inserted} | ROWS_UPDATED=${rows_updated} | EXEC_MS=${execution_time} | RUN_ID=${RUN_ID}'
    ...
    _log_df_Log_Audit_Complete.info('AUDIT | EVENT=AUDIT_COMPLETE | TRANS=TR_Sales_Audit | ROWS_READ=${rows_read} | ROWS_WRITTEN=${rows_written} | ROWS_REJECTED=${rows_rejected} | ROWS_INSERTED=${rows_inserted} | ROWS_UPDATED=${rows_updated} | EXEC_MS=${execution_time} | RUN_ID=${RUN_ID}')
