"""Transformation module for missing Pentaho file: Reporting_Monthly_Sales.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.reporting_monthly_sales")

from pyspark.sql import types as T


SOURCE_KTR = 'Reporting_Monthly_Sales.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Reporting_Monthly_Sales.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
