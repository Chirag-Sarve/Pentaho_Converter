"""Transformation module for missing Pentaho file: Promotion_Cleansing.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.promotion_cleansing")

from pyspark.sql import types as T


SOURCE_KTR = 'Promotion_Cleansing.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Promotion_Cleansing.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
