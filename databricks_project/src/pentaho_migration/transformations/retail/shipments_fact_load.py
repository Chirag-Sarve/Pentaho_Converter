"""Transformation module for missing Pentaho file: Shipments_Fact_Load.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.shipments_fact_load")

from pyspark.sql import types as T


SOURCE_KTR = 'Shipments_Fact_Load.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Shipments_Fact_Load.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
