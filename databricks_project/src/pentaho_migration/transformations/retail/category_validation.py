"""Transformation module for missing Pentaho file: Category_Validation.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.category_validation")

from pyspark.sql import types as T


SOURCE_KTR = 'Category_Validation.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Category_Validation.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
