"""Transformation module for missing Pentaho file: Employee_Cleansing.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.employee_cleansing")

from pyspark.sql import types as T


SOURCE_KTR = 'Employee_Cleansing.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Employee_Cleansing.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
