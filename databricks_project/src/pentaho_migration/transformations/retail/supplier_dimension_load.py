"""Transformation module for missing Pentaho file: Supplier_Dimension_Load.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.supplier_dimension_load")

from pyspark.sql import types as T


SOURCE_KTR = 'Supplier_Dimension_Load.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Supplier_Dimension_Load.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
