"""Transformation module for missing Pentaho file: Reporting_Daily_Sales.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.reporting_daily_sales")

from pyspark.sql import types as T


SOURCE_KTR = 'Reporting_Daily_Sales.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Reporting_Daily_Sales.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
