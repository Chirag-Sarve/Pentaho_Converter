"""Transformation module for missing Pentaho file: Store_Cleansing.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.store_cleansing")

from pyspark.sql import types as T


SOURCE_KTR = 'Store_Cleansing.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Store_Cleansing.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
