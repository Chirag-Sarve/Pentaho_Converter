"""Transformation module for missing Pentaho file: Category_Dimension_Load.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.category_dimension_load")

from pyspark.sql import types as T


SOURCE_KTR = 'Category_Dimension_Load.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Category_Dimension_Load.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
