"""Transformation module for missing Pentaho file: Supplier_Cleansing.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.supplier_cleansing")

from pyspark.sql import types as T


SOURCE_KTR = 'Supplier_Cleansing.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Supplier_Cleansing.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
