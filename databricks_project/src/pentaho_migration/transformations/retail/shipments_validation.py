"""Transformation module for missing Pentaho file: Shipments_Validation.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.shipments_validation")

from pyspark.sql import types as T


SOURCE_KTR = 'Shipments_Validation.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Shipments_Validation.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
