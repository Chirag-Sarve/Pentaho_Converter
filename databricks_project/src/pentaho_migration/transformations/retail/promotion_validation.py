"""Transformation module for missing Pentaho file: Promotion_Validation.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.promotion_validation")

from pyspark.sql import types as T


SOURCE_KTR = 'Promotion_Validation.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Promotion_Validation.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
