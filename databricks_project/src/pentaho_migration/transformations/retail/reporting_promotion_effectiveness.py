"""Transformation module for missing Pentaho file: Reporting_Promotion_Effectiveness.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.reporting_promotion_effectiveness")

from pyspark.sql import types as T


SOURCE_KTR = 'Reporting_Promotion_Effectiveness.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Reporting_Promotion_Effectiveness.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
