"""Transformation module for missing Pentaho file: Reporting_Inventory_Summary.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.reporting_inventory_summary")

from pyspark.sql import types as T


SOURCE_KTR = 'Reporting_Inventory_Summary.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Reporting_Inventory_Summary.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
