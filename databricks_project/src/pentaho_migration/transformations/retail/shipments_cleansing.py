"""Transformation module for missing Pentaho file: Shipments_Cleansing.ktr
...
_LOG = get_logger("pentaho_migration.transformations.retail.shipments_cleansing")

from pyspark.sql import types as T


SOURCE_KTR = 'Shipments_Cleansing.ktr'
SOURCE_MISSING = True
...
def run(spark: SparkSession, config: Mapping[str, Any] | None = None) -> DataFrame:
    ...
    return spark.createDataFrame(
        [('Shipments_Cleansing.ktr', config.get("RUN_ID"))],
        schema=schema,
    )
